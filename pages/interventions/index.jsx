...
const interventions = [
  {
    id: 1,
    title: 'Testosterone',
    text: 'Cone decongestants farsighted glycemic index lunula pediatric endocrinologist.',
    list: ['Magnesium', 'Boron', 'Vitamin D'],
  },
  {
    id: 2,
    title: 'Anxiety',
    text: 'Arthritis certified diabetes educators (cdes) depression hemangioma humidifier.',
    list: ['Creatine', 'Copper', 'Maca Powder'],
  },
  {
    id: 3,
    title: 'Women’s Health',
    text: 'Tube gastric juices genetics occupational therapist papillae rescue medications .',
    list: ['Folic Acic', 'Iron ', 'Vitamin B12'],
  },
  {
    id: 4,
    title: 'Men’s health',
    text: 'System neuropathy operation otolaryngologist sebum sulfites vertebrae. ',
    list: ['Magnesium', 'Boron', 'Vitamin D'],
  },
  {
    id: 5,
    title: 'Fertility',
    text: 'Tube gastric juices genetics occupational therapist papillae rescue.',
    list: ['Magnesium', 'Boron', 'Vitamin D'],
  },
  {
    id: 6,
    title: 'Skin and Nails',
    text: 'Arthritis certified diabetes educators (cdes) depression hemangioma humidifier.',
    list: ['Magnesium', 'Boron', 'Vitamin D'],
  },
]
        ...
        <div className="mx-auto flex 2xl:justify-end wide:max-w-fixed">
          <div className="w-full max-w-[1085px] px-6 py-10 md:px-12 lg:pb-18 lg:pt-10 xxl:px-19">
            ...
            <div className="space-y-3 text-white lg:mt-5">
              <h1 className="font-lora">Interventions</h1>
              <p className="p-xl max-w-3xl -tracking-2">
                All our interventions pages are backed with references to scientific papers. We
                currently have over 55,000 references.
              </p>
            </div>
          </div>
      ...
      <div className="mx-auto w-full wide:max-w-fixed">
        <div className="flex justify-end">
          ...
          <div className="z-10 w-full space-y-25 bg-white px-6 py-10 shadow-form md:py-14 md:px-12 xl:max-w-[1085px] xxl:px-19">
            {/* Popular Interventions by Condition */}
            <div>
              <h2 className="font-lora">
                Popular Interventions <span className="hidden md:inline">by Condition</span>
              </h2>

              <div className="mt-8 grid gap-10 md:grid-cols-2 md:gap-14 lg:mt-16 lg:grid-cols-3">
                {interventions.map(({ id, title, text, list }) => (
                  <div key={id}>
                    <div>
                      <img
                    ...
                    <h4 className="mt-3 pr-10 font-lora font-medium lg:mt-4">{title}</h4>
                    <p className="mt-2 leading-7 text-gray-600 lg:mt-4 lg:text-lg">{text}</p>
                    ...
                    <Link href={`/interventions/${title.toLowerCase().split(' ').join('-')}`}>
                  ...
                  <div key={index} id={list.alphabet}>
                    ...
                    <div className="mt-6 gap-5 md:columns-3 lg:mt-8">
                      {list.interventions.map((item, idx) => (
                        <Link href={`/interventions/${item.name}`} key={idx}>
                          <div className="mb-5 flex items-center [&:nth-child(n+7)]:hidden md:[&:nth-child(n+7)]:block">
                            <span>{item.name}</span>
                            {item.new && (
...
export const getServerSideProps = async () => {
  const res = await axios.get(`https://api.dev.examinenew.com/v1/interventions`)
  console.log(res)
  return {
    props: { data: res.data },
  }
